...
export function WhereToFind({ locale }: { locale: Locale }) {
  ...
  return (
    <section
      ...
      style={{
        backgroundImage:
          "linear-gradient(90deg, rgba(12, 10, 8, 0.9), rgba(12, 10, 8, 0.7) 55%, rgba(12, 10, 8, 0.88)), url('/images/mundus-where-to-find-background.webp')",
      }}
    >
      <div className="mundus-container relative z-10">
        ...
        <div className="mx-auto mt-10 grid max-w-5xl gap-4 lg:grid-cols-[0.9fr_1.1fr] sm:mt-12">
          <address className="flex min-h-80 flex-col justify-between border border-cream/20 bg-onyx/65 p-8 not-italic backdrop-blur-sm sm:p-10">
            <div>
              <p className="text-xs font-bold tracking-[0.14em] text-honey">
                MUNDUS LOUNGE
              </p>
              <p className="mt-6 font-display text-3xl font-[200] leading-tight tracking-[-0.04em] text-cream">
          ...
          <div className="border border-honey/80 bg-onyx/70 p-8 backdrop-blur-sm sm:p-10">
            <h3 className="font-display text-3xl font-[200] tracking-[-0.04em] text-cream">
              {messages.openingHours}
            </h3>
            <dl className="mt-7 divide-y divide-cream/15">
              {business.openingHours.map((hours) => (
                <div className="flex items-center justify-between gap-6 py-3" key={hours.day}>
                  <dt className="text-sm text-cream/80 sm:text-base">
                    {messages.days[hours.day] ?? hours.day}
                  </dt>
                  <dd className="text-right text-sm font-medium text-cream sm:text-base">
